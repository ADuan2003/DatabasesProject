...
(EmployeeID TEXT NOT NULL,
  ...
  Coverage TEXT NOT NULL,
  CHECK (HiringSalary >= A FROM SELECT(MIN(minSalary) as A FROM Positions WHERE position = HiringPosition),
  CHECK (HiringSalary <= B FROM SELECT(MAX(maxSalary) as B FROM Positions WHERE position = HiringPosition),
  CHECK (CurrentSalary >= C FROM SELECT(MIN(minSalary) as C FROM Positions WHERE position = CurrentPosition),
  CHECK (CurrentSalary <= D FROM SELECT(MAX(maxSalary) as D FROM Positions WHERE position = CurrentPosition),
...
def query2():
  ...
  if request.method == 'POST':
    try:
      ...
      con.row_factory = sqlite3.Row
      cursor = con.cursor()
      cursor.execute('SELECT DISTINCT e.EmployeeID FROM EmployeeInfo e, EmployeeAssignments d WHERE e.employeeID = d.employeeID and (d.EndDate IS NULL OR d.EndDate >= date()) and d.Department = ?', (dept,))
      info = cursor.fetchall()
      cursor.close()
    except Exception as e:
      return render_template('error.html', e=e)
...
def query7():
  ...
  if request.method == 'POST':
    try:
      ...
      con.row_factory = sqlite3.Row
      cursor = con.cursor()
      cursor.execute('SELECT COUNT (DISTINCT EmployeeID) FROM EmployeeBenefits WHERE (EndDate IS NULL OR EndDate >= date()) AND Benefit = ?', (ben,))
      info = cursor.fetchall()
      cursor.close()
    except Exception as e:
      return render_template('error.html', e=e)
